(function ($, $document) {
    var EAEM_PLUGIN_ID = "eaem-dyn-var",
        ...
        DYN_VAR_SELECTOR_URL = "/apps/efe/clientlibs/cfm-personalization-token-plugin/personalization-token-selector.html",
    ...
    function getWidgetValue(form, selector, value, enable){
    ...
    function sendSelectedVars(){
        ...
        var inputName = "./style";
        var inputField = $form.find(`input[name="${inputName}"]`);
        if (inputField.length > 0) {
            var inputValue = decodeURIComponent(inputField.val());
            var fieldName = inputName.substr(2);
            message.data[fieldName] = inputValue;
        }
