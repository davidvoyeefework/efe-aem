 (function() {
    ...
    function onDocumentReady() {
        ...
        function getUTMValues () {
            let utmParamArray = ["utm_source","utm_medium","utm_campaign","utm_placement","utm_device","utm_term","utm_content"];
        ...
        function getVoyaValues () {
            let voyaParamArray = ["clickthrough","pagecode","treatment","alertDecision","lastAlert","explatWPI","destination",
                "placement","nextGenWPI","service","rkUrl","shownDisclaimer", "deeplink"];
            let voyaArray = [];
            for(let i = 0; i < voyaParamArray.length; i++) {
                if(urlParams.has(voyaParamArray[i])) {
                    voyaArray.push(voyaParamArray[i] + "=" + urlParams.get(voyaParamArray[i]));
                }
            }
            var voyaList = "";
